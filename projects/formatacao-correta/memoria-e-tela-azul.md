...
## Intervencao nao destrutiva em ferramenta opcional Intel — 23/09 (EV-21)

Desktop Commander conectado ao notebook confirmou Intel Computing Improvement Program em dois registros MSI/bundle, `semav6msr64.sys` ativo e servicos QUEENCREEK ativos; eventos SCM 7034 em 21 e 23/09. Criado ponto de restauracao. Desinstalacao oficial dos dois registros retornou **0**; verificacao posterior confirmou ausentes programa, driver, arquivo, pasta e servicos QUEENCREEK. `iaStorAC`, `MEIx64` e `PawnIO` permaneceram ativos; `verifier /querysettings` permaneceu 0. Sem reboot. **Isto encerra a remocao do componente Intel CIP, NAO a investigacao da causa do 0x1A**, pois ambos os dumps continuaram sem escritor identificado. Nao repetir a desinstalacao e nao ativar Driver Verifier por inercia. [EV-21](./registro-de-evidencias.md) · [STATE](../../memory/STATE.yaml).
