...
## Novo erro conjunto de armazenamento e servico — 23/09 21h46 (EV-31)

`iaStorAC Event129` 21:46:19 reportou timeout/reset `\\Device\\RaidPort0`; SCM 7000/7009 21:46:20 para `AppXSvc` com timeout de 30s, 1s de separação no carimbo do evento. **Pode haver gargalo compartilhado de I/O, mas nao afirmar causalidade definitiva.** `Start-Service AppXSvc` em 22:19:53 obteve SUCESSO, status Running (modo Manual mantido); nao houve novo 129/7000/7009 no reteste. O estado Stopped/Manual antes do teste nao era defeito isolado, pois o servico e sob demanda. A trilha SATA agora esta REABERTA em [armazenamento](./armazenamento-e-whea.md). Na consulta conjunta de erros recentes, ScreenClippingHost permaneceu quebrado (inclusive 22h11), WinDbg reportou 0x80270301; SearchApp caiu 19h17. AppModel-State PC Manager repetiu eventos 20/21/23/24 ate 16h39, mesmo instalado Status Ok; falta verificar se erro funcional persiste na versao instalada antes de Reset-AppxPackage, que apaga preferencias de usuario. [EV-31/34](./registro-de-evidencias.md).

## Nova confirmacao local (23/09 ~22h36–22h44, EV-36)

Verificação SFC SOMENTE de `combase.dll` e `ScreenClipping.dll` retornou exit 0/sem violacao; ambas e `ScreenClippingHost.exe` com assinatura Microsoft válida. `ms-screenclip` está presente em HKCU e pacote CBS possui `ScreenClipping` no manifesto. ACL do grupo ALL APPLICATION PACKAGES com ReadKey foi conferida em HKU, HKLM/Software/Classes, HKCU/Software/Classes, ms-screenclip e ActivatableClasses; as pastas Windows, Program Files e SystemApps CBS já tinham ReadAndExecute. Assim, o caso NAO deve receber correção genérica das permissões raiz ou repetição SFC/DISM. `MicrosoftWindows.Client.CBS` tem `TextInputHost` rodando, portanto reset de estado do pacote afetaria mais do que a captura e não foi realizado. Em 21h11 houve ainda um evento AppXDeploymentServer 404 `0x8007042B` para CBS, registrado no mesmo intervalo do re-registro; **a existência desse evento nao prova qual operacao especifica gerou a falha nem substitui a validação do app**, que continuou caindo. [EV-36](./registro-de-evidencias.md).

### Atualizacao Shell nao resolveu — 23/09 22h49, EV-39

Apos o re-registro CBS anterior, foi disparada via `shell32!SHChangeNotify` a notificacao **SHCNE_ASSOCCHANGED + SHCNF_FLUSH**, conforme [Microsoft](https://learn.microsoft.com/en-us/windows/win32/shell/reg-shell-exts) para novas associacoes Shell. A reativacao `ms-screenclip:` gerou **OUTRO Application1000 as 22h48:54**; preferencia `PrintScreenKeyForSnippingEnabled=0` preservada. Logo a hipótese de simples cache de associacao Shell desatualizada NAO resolveu; encerrar esse teste, não repetir por tentativa. Captura segue caso ABERTO até uma causa COM/WinRT específica ser localizada.
